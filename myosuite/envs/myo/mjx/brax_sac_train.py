...
"""Brax SAC with training progress independent of evaluation frequency.

Adapted from Brax 0.14.1, brax/training/agents/sac/train.py (Apache-2.0).
Local changes: bounded training/prefill chunks and scalar progress callbacks.
Networks, losses, replay, updates, evaluation and checkpointing remain Brax's.
...
def _chunk_sizes(total_steps, completed_steps, log_interval):
  """Split vector steps at global logging boundaries, including a final tail."""
  while total_steps > 0:
    size = min(total_steps, log_interval - completed_steps % log_interval)
    yield size
...
def train(
  ...
  """SAC training; log_interval counts vector steps, including replay prefill."""
  if log_interval < 1:
  ...
  def prefill_replay_buffer(
    ...
    # A dynamic bound reuses the executable for partial logging/eval intervals.
  ...
  prefill_replay_buffer = jax.pmap(
      ...
      donate_argnums=(0, 1, 2), in_axes=(0, 0, 0, 0, None)
  )
  ...
  def training_epoch(
      ...
      key: PRNGKey,
      num_steps: int,
  ) -> Tuple[TrainingState, envs.State, ReplayBufferState, PRNGKey, Metrics]:

    def f(unused_t, carry):
      ts, es, bs, k, totals = carry
      k, new_key = jax.random.split(k)
      ts, es, bs, metrics = training_step(ts, es, bs, k)
      totals = jax.tree_util.tree_map(
          lambda total, value: total + jnp.mean(value), totals, metrics
      )
      return ts, es, bs, new_key, totals
    ...
    training_state, env_state, buffer_state, key, totals = jax.lax.fori_loop(
        0, num_steps, f,
        (training_state, env_state, buffer_state, key, totals),
    )
    metrics = jax.tree_util.tree_map(lambda total: total / num_steps, totals)
  ...
  def training_epoch_with_timing(
    ...
    t = time.time()
    (training_state, env_state, buffer_state, key, metrics) = training_epoch(
        training_state, env_state, buffer_state, key, num_steps
    )
    ...
    sps = (
        env_steps_per_actor_step * num_steps
    ) / max(epoch_training_time, 1e-8)
    metrics = {
        'training/sps': sps,
        'training/walltime': training_walltime,
        'training/vector_steps': int(_unpmap(training_state.env_steps))
  ...
  for chunk_size in _chunk_sizes(num_prefill_actor_steps, 0, log_interval):
    t = time.time()
    training_state, env_state, buffer_state, prefill_keys = prefill_replay_buffer(
        training_state, env_state, buffer_state, prefill_keys, chunk_size
    )
    jax.block_until_ready((training_state, env_state, buffer_state, prefill_keys))
    elapsed = time.time() - t
    ...
    if process_id == 0:
      progress_fn(int(_unpmap(training_state.env_steps)), {
          ...
          'training/gradient_steps': int(_unpmap(training_state.gradient_steps)),
          'training/warmup': 1,
          'training/buffer_current_size': jnp.sum(
              jax.vmap(replay_buffer.size)(buffer_state)) * jax.process_count(),
          'training/sps': chunk_size * env_steps_per_actor_step / max(elapsed, 1e-8),
          'training/walltime': training_walltime,
      })
  ...
  for _ in range(num_evals_after_init):
    ...
    for chunk_size in _chunk_sizes(
        num_training_steps_per_epoch, completed_vector_steps, log_interval
    ):
      training_state, env_state, buffer_state, epoch_keys, training_metrics = (
          training_epoch_with_timing(
              training_state, env_state, buffer_state, epoch_keys, chunk_size
          )
      )
      completed_vector_steps += chunk_size
      epoch_steps += chunk_size
      current_step = int(_unpmap(training_state.env_steps))
      # Merge the final chunk with evaluation below, avoiding two W&B records
      # with the same step. Preserve the PRNG carry within the original epoch.
      if process_id == 0 and epoch_steps < num_training_steps_per_epoch:
